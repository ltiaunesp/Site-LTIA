...
class zerif_clients_widget extends WP_Widget
{
    ...
    function zerif_clients_widget()
    {
        ...
        $widget_ops = array('classname' => 'zerif_clients');
         ...
         $this->WP_Widget('zerif_clients-widget', 'Parceiros', $widget_ops);
...
class MeuWidget extends WP_Widget{
	...
	public function widget($argumentos, $instancia){
		...
		<section class="project_block">
			<h3 class="project_title"><?php echo @$instancia["meu_titulo"] != "" ? $instancia["meu_titulo"] : "Meu projeto";?></h3>
			<p class="project_description"><?php echo @$instancia["meu_campo"] != "" ? $instancia["meu_campo"] : "O projeto consiste nisso nisso e nisso";?></p>
			<span class="project_see_more">[+]</span>
		</section>
		<?php
	}
	...
	public function form($instancia){
		...
		?>
			...
			<p>
				...
				<textarea id="<?php echo self::get_field_id('meu_campo');?>" name="<?php echo self::get_field_name('meu_campo');?>"  placeholder="Digite aqui seu texto" ><?php echo $widget["meu_campo"];?></textarea>
			</p>
		<?php
	}
...
/*
// Sistema para utilizar layouts especificos de cada post
function get_category_template_path($t){
	var_dump($t);
...
class LocalConfig{
	...
	function __construct(){
		add_filter('admin_init', array($this,'registraCampo'));
	}
	function registraCampo(){
		register_setting('general', $this->IDCAMPO);
		add_setting_field('fav-color','<label for="'.$this->IDCAMPO.'">Endere&ccedil;o: </label>',array($this,'criaCampo'),'general');
	}
	function criaCampo(){
		$val = get_option(IDCAMPO,'');
		?>
		<input type="text" name="<?php echo $this->IDCAMPO; ?>" value='<?php echo $val;?>' id="<?php echo $this->IDCAMPO; ?>"/>
...
// CONTAINS FUNCTION
	...
	function addSvgToGallery($mimes){
	...
	class socialSettings {
		...
		function register_fields() {
			global $_redes;
			foreach($_redes as $rede){
				register_setting( 'general', $rede.'_field', 'esc_attr' ); // Registrando campo
			}
		...
		function fields_html() {
			...
			foreach($_redes as $rede){
				// Campo
				$value = get_option($rede.'_field' , '');
				echo "<input class='regular-text' type='text' id='{$rede}_field' placeholder='". ucwords($rede) ."' name='{$rede}_field' value='{$value}' />";
				echo '<p class="description" id="'.$rede.'-description">URL do '.ucwords($rede).'</p>';
				echo "<br>";
			}
		}
	}
